...
This leads us to add to the basic model a movie effect, so nos our new model is given by \ref{eq:one}:
...
To model this effect, a penalized cubic splines is selected (see [@wood_generalized_2006] and [@james_introduction_2013]. A cubic splines is used instead of a smoothing spline or loess for computational reasons, and penalized are used given that older movies, though on average are better rated, have much fewer ratings, therefore higher variance. The model is also fit to the timestamp variable.
...
# Getting RMSE with validation set.
