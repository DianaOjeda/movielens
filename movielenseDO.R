...
#Create auxiliary variables from timestamp
...
# note this is NOT the validation set s
#####################################
...
#Two loops are run in order to find the k parameter (number of knots) in the 
#spline model that mimimizes the error on the test set.
# The second one uses a finer grid, from the results obtained
# in the first. 
### Each loop takes several hours to complete
...
ks[which.min(errors_spline1),]
# Find values of k in finer grid, based on previous result
...
ks2[which.min(errors_spline2),]  #values of k that minimize rmse on test set.

#Fit a final model for the RESIDUALS with the correct values for K, obtained in the 
#previous loop.
